    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 18, marginBottom: 24 }}>
        {plans.map((p, i) => {
          ...
          return (
            <div key={p.id} data-testid={`plan-${p.id}`} className={featured ? "glass fade-up" : "clay fade-up"} style={{ padding: 26, animationDelay: `${i * 70}ms`, border: featured ? `2px solid ${accent}` : "2px solid transparent", position: "relative" }}>
              {featured && <span className="chip chip-accent" style={{ position: "absolute", top: 18, right: 18 }}>Popular</span>}
              <div style={{ width: 44, height: 44, borderRadius: 13, background: `${accent}1f`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
                <Icons.Crown size={22} color={accent} />
              </div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-2)" }}>{p.name}</div>
              <div className="display" style={{ fontSize: 34, fontWeight: 800, margin: "6px 0" }}>
                ₹{p.price_inr.toLocaleString("en-IN")}
              </div>
              <div style={{ fontSize: 13, color: "var(--text-3)", marginBottom: 18, minHeight: 34 }}>{p.blurb}</div>
